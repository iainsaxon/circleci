...
class Low_search extends Low_search_base {
	...
	public function filters()
	{
		...
		if ($get_collections !== FALSE)
		{
			...
			// Init by site ids
			$collections = ee()->low_search_collection_model->get_by_site($this->params->site_ids());

			// Handle show="foo"
			if (isset($get_collections['show']))
			{
				$collections = ee()->low_search_collection_model->get_by_param($get_collections['show'], $collections);
			}
	...
	public function form()
	{
		...
		foreach (array_keys(low_array_get_prefixed(ee()->TMPL->tagparams, 'form_', TRUE)) AS $key)
		{
	...
	public function results()
	{
			...
			! preg_match('#/P\d+/?$#', ee()->uri->uri_string()))
		{
			...
			// Also set cache
			low_set_cache(LOW_SEARCH_PACKAGE, ee()->low_search_log_model->key, $this->log_id);
		}
	...
	public function collections()
	{
		...
		{
			$rows = ee()->low_search_collection_model->get_by_param($val, $rows);
		}
	...
	public function param($which = FALSE)
	{
		...
		if ($as = ee()->TMPL->fetch_param('as'))
		{
			...
			foreach ($vals AS $val)
			{
				$vars[] = array($as => low_format($val, $format));
			}
	...
	public function catch_search()
	{
		...
		if ($this->_log_search($data))
		{
			...
			ee()->session->set_flashdata(ee()->low_search_log_model->key, $this->log_id);
		}
	...
	private function _log_search($data = array())
	{
		if (($search_log_size = $this->settings->get('search_log_size')) !== '0' && is_numeric($search_log_size))
		{
			...
			// Log search
			$this->log_id = ee()->low_search_log_model->insert(array(
				'site_id'     => $this->site_id,
	...
	private function _no_results($tagdata = NULL)
	{
		...
		if ($this->log_id)
		{
			ee()->low_search_log_model->add_num_results(0, $this->log_id);
		}
	...
	private function _channel_entries()
	{
		...
		if ($this->log_id)
		{
			...
			if ( ! $total && ($entry_ids = low_get_cache('channel', 'entry_ids')))
			{
			...
			if ($total)
			{
				ee()->low_search_log_model->add_num_results($total, $this->log_id);
			}
		}
