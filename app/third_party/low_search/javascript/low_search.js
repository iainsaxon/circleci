...
LOW.Index = function(cell) {
	...
	var update = function(start, status, xhr) {
		var width, text, done;

		if (start === false) {
			done  = true;
			width = 'auto';
			text  = 'An error occurred rebuilding the index. Try again later or contact support.';
		} else if (start === true) {
			done  = true;
			width = 'auto';
			text  = lang('done');
		...
		if (done) {
			...
			// If we're not done, we're still building
	...
	var buildBatch = function(rebuild) {
		...
		$.post(url, vars, update, 'json').error(function(){
			$bar.text('An error occurred rebuilding the index. Try again later or contact support.');
