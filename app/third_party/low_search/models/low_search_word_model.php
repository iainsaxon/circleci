 ...
 */
class Low_search_word_model extends Low_search_model {
	...
	function __construct()
	{
		...
		$this->_first_letter_suggestions = (bool) ee()->config->item('low_search_first_letter_suggestions');

		// --------------------------------------
		// Optionally override the Levenshtein method via config file
		// --------------------------------------

		$method = ee()->config->item('low_search_levenshtein_method');
	...
	public function get_unknown($words, $lang, $site)
	{
		...
		if ( ! is_array($site)) $site = array($site);
		...
		// Get known words
		$known = low_flatten_results($query->result_array(), 'word');
	...
	public function get_dirty($words, $lang, $site)
	{
		...
		if ( ! $site) $site = array($this->site_id);
	...
	public function get_lang_count()
	{
		...
		// Return the flatness
		return low_flatten_results($query->result_array(), 'num', 'language');
	}
	 ...
	 * @return     array
	 */
	public function delete($word, $lang)
	{
	...
	public function get_suggestions($words, $lang, $site = NULL, $distance = 2, $limit = 5)
	{
		...
		if (empty($site)) $site = $this->site_id;
	...
	private function _php_suggestions($words, $lang, $site, $distance, $limit)
	{
		...
		foreach ($words AS $word)
		{
			...
			$first = $this->_first_letter_suggestions
				? ee()->low_multibyte->substr($word, 0, 1)
				: '0';
			...
			if ( ! isset($cache[$key]))
			{
				...
				$cache[$key] = low_flatten_results($query->result_array(), 'word');
			}
	...
	private function _sql_suggestions($words, $lang, $site, $distance, $limit)
	{
		...
		foreach ($words AS $word)
		{
			...
			if ($this->_first_letter_suggestions)
			{
				ee()->db->like('clean', ee()->low_multibyte->substr($word, 0, 1), 'after');
			}

			$query = ee()->db->get();

			// Suggestions
			$suggestions = low_flatten_results($query->result_array(), 'word');
...
/* End of file Low_search_word_model.php */