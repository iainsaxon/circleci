...
class Assets_ft extends EE_Fieldtype
{
		...
		foreach ($paths as $sort_order => $path)
		{
			...
			if (preg_match('/^\{filedir_(\d+)\}(.*\/)?([^\/]+)$/', $path, $match))
			{
				...
				if (empty($file_id))
				{
					...
					// register it.
					$file_id = $this->EE->assets_lib->register_file($parent_id, $filename);
				}
				else
				{
	...
	function save($file_ids)
	{
		...
		{
			$filedir_id = $this->EE->input->post($field_name .'_filedir');
			$source_id = $this->EE->input->post($field_name .'_source');
	...
	private function _simple_html_upload($file_info, $filedir_id = null, $source_id = null)
	{
		...
		$folder_id = $this->EE->assets_lib->get_folder_id_by_params(
			$params
		);
		...
		try
		{
			...
			else
			{
				...
				$source = $this->EE->assets_lib->instantiate_source_type($source_row);
			}

			$file_name = $this->EE->assets_lib->clean_filename($file_name);
