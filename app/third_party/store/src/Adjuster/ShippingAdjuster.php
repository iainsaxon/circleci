...
class ShippingAdjuster extends AbstractAdjuster
{
    public function adjust(Order $order)
    {
        ...
        $shippingOverride = $order->shipping_override;
        if (isset($shippingOverride['shipping_name']) && $shippingOverride['shipping_name']) {
            $adjustment = new OrderAdjustment();
            $adjustment->name = $shippingOverride['shipping_name'];
            $adjustment->type = 'shipping';
            $adjustment->amount = $shippingOverride['shipping_amount'];
            $adjustment->taxable = 1;
            $adjustment->included = 0;
