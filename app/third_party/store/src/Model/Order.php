...
class Order extends AbstractModel
{
    ...
    public function getTaxOverrideAttribute($value)
    {
    ...
    public function setTaxOverrideAttribute($value)
    {
        $this->attributes['tax_override'] = json_encode($value);
    }

    public function getShippingOverrideAttribute($value)
    {
        $modifiers = json_decode($value, true);
    ...
    public function setShippingOverrideAttribute($value)
    {
        $this->attributes['shipping_override'] = json_encode($value);
    }
    ...
    public function addItem(array $item_attributes, array $form_params = array())
    {
        ...
        if ($this->exists) {
            if (isset($item_attributes['item_id']) && $item_attributes['item_id'] != false) {
                $item = OrderItem::find($item_attributes['item_id']);
            } else {
        ...
        // We need this for MCP Order Edit
        return $item;
    }
    ...
    public function recalculate()
    {
        ...
        // Make sure the order_total is not a negative number
        // This might not play so well if you changed the order of the adjusters.
    ...
    public function markAsComplete()
    {
        ...
        $highestOrderId = ee()->store->db->table('store_orders')->select(array('order_id'))->where('site_id', $this->site_id)->where('order_id', '>', 0)->orderBy('order_id', 'DESC')->first();
        if ($highestOrderId) $highestOrderId = $highestOrderId['order_id']+1;
        else $highestOrderId = config_item('store_order_id_start');

        $this->order_id = $highestOrderId;
    ...
    public function toTagArray()
    {
        ...
        if ($this->discount) {
            ...
            $attributes['discount:code'] = $this->discount->code;
            $attributes['discount:base_discount'] = store_currency($this->discount->base_discount);
            ...
            $attributes['discount:base_discount'] = null;
