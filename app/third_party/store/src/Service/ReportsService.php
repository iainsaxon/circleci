...
class ReportsService extends AbstractService
{
    ...
    {
        $query = $this->ee->store->db->table('members');
        $query->select(array('members.member_id', 'members.screen_name'));
        $query->join('store_orders', 'store_orders.member_id', '=', 'members.member_id');
        $query->where('order_completed_date', '>', 0);
        $query->groupBy('members.member_id');
