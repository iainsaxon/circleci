...
class PaymentsService extends AbstractService
{
    ...
    public function process_payment(Order $order, $transaction, $card_data, $send = true)
    {
        ...
        $card = $this->build_payment_credit_card($order, $card_data);
    ...
    public function complete_payment(Transaction $transaction)
    {
        ...
        if ($gateway->$supportsAction()) {
            ...
            if ($transaction->payment_method == 'Mollie_Ideal' || $transaction->payment_method == 'Mollie') {
                $params['transactionReference'] = $transaction->reference;
            }

            // If SagePay, we need the actual reference
            if ($transaction->payment_method == 'SagePay_Server') {
                $params['transactionReference'] = $transaction->reference;
            }
    ...
    public function build_payment_request(Transaction $transaction)
    {
        ...
        $request['transactionReference'] = $transaction->hash;
    ...
    {
        // some gateways can't handle a return URL with a query string
        // we implement some custom routing to avoid this issue
        $no_query_string_gateways = array('PaymentExpress_PxPay');

        if (in_array($transaction->payment_method, $no_query_string_gateways)) {
            return $this->ee->store->store->create_url('payment_return/'.$transaction->hash);
        } else {
