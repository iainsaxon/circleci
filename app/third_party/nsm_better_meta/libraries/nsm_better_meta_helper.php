 ...
 * $this->EE->nsm_better_meta_helper->addCSS('custom_field.css');
 * 
 * # Load the JS for the iframe
 * $this->EE->nsm_better_meta_helper->addJS('custom_field.js');
 * $this->EE->nsm_better_meta_helper->addJS('../lib/jquery.cookie.js');
 * 
...
class Nsm_better_meta_helper{
	...
	private function _getThemeUrl()
	{
      ...
      $url_third_themes = get_instance()->config->item('theme_folder_url');
      if (substr($url_third_themes, -1) != '/') {
        $url_third_themes .= '/';
      }
      $url_third_themes .= "third_party/";
      define('URL_THIRD_THEMES', $url_third_themes);
    }
 	...
 	public function checkbox($input_name, $input_value, $checked, array $options = array())
	{
		...
		$checked = ($checked == $input_value) ? "checked='checked'" : "";
	...
	public function yesNoRadioGroup($input_name, $checked_value, $options = array())
	{
		...
		$checked_str = ($checked_value == $flags[0]) ? " checked='checked' " : FALSE;
		$ret = '<label for="'.$input_id.'_yes">';
		$ret .= '<input type="radio" value="'.$flags[0].'" '.$checked_str.' id="'.$input_id.'_yes" '. $attributes_str . ' />';
		$ret .= 'Yes </label>';

		$checked_str = ($checked_value == $flags[1]) ? " checked='checked' " : FALSE;
		$ret .= '<label for="'.$input_id.'_no">';
		$ret .= '<input type="radio" value="'.$flags[1].'" '.$checked_str.' id="'.$input_id.'_no" '. $attributes_str . ' />';
		$ret .= 'No </label>';
