...
class Zenbu_db extends Zenbu_mcp {
		...
		*/
		if ($this->EE->extensions->active_hook('zenbu_add_column') === TRUE)
		{
		 	...
		 	if(is_array($hook_fields_and_labels))
			{
				...
				{
					if(isset($fal['settings']))
					{
						foreach($fal['settings'] as $setting_key => $setting_val)
						{
							$this->non_ft_extra_options[$setting_key] = $setting_key;
						}
					}
				}
			}
		...
		$ft_option_labels_array = array_merge($ft_option_labels_array, $this->non_ft_extra_options);
	...
	*/
	...
	/**
	 * save_cached_filters - Cache filter data for later use
	 * @param  array $filter_data An array of filter data, eg. rules, limit, etc
	 * @return void              Data is saved to exp_zenbu_filter_cache
	 */
	public function save_cached_filters($filter_data)
	{
		$db_data['member_id']		= $this->member_id;
		$db_data['site_id']			= $this->site_id;
		$db_data['save_date']		= $this->EE->localize->now;
		$db_data['filter_rules']	= serialize($filter_data);

		$sql = $this->EE->db->insert_string('exp_'.$this->addon_short_name.'_filter_cache', $db_data);
		$this->EE->db->query($sql);

	} // END save_cached_filters()

	// --------------------------------------------------------------------


	/**
	 * clean_up_filter_cache - Remove old temp filter cache data
	 * @return void              Table is cleaned up
	 */
	public function clean_up_filter_cache()
	{
		$old_time = $this->EE->localize->now - 14400;
		$this->EE->db->query("DELETE FROM exp_zenbu_filter_cache WHERE member_id = " . $this->member_id . " AND site_id = " . $this->site_id . " AND save_date < " . $old_time); // 4 hours
