...
class Zenbu_get extends Zenbu_mcp {
	...
	function __construct()
	{
		...
		//	----------------------------------------
		// 	First filter rule dropdown options
		//	----------------------------------------
		$this->first_dropdown  = array(
		...
		//	----------------------------------------
		// 	Second filter rule dropdown options - per type
		//	----------------------------------------
		$this->channel_second_dropdown 	= array(
			'is' 				=> 'is'
		);
		$this->second_dropdown_type1 	= array(
			'is'    			=> $this->EE->lang->line('is'),
			'isnot' 			=> $this->EE->lang->line('isnot')
		);
		$this->second_dropdown_type2 	= array(
			'contains' 			=> $this->EE->lang->line('contains'),
			'doesnotcontain' 	=> $this->EE->lang->line('doesnotcontain'),
		);
		$this->second_dropdown_type3 	= array(
			'contains' 			=> $this->EE->lang->line('contains'),
			'doesnotcontain' 	=> $this->EE->lang->line('doesnotcontain'),
		...
		//	----------------------------------------
		$this->limit_dropdown_options 	= array(
			//'1' => '1'.NBS.$this->EE->lang->line('result'),
			//'2' => '2'.NBS.$this->EE->lang->line('results'),
			'5' 	=> $this->EE->lang->line('show').NBS.'5'.NBS.$this->EE->lang->line('results'),
			'10' 	=> $this->EE->lang->line('show').NBS.'10'.NBS.$this->EE->lang->line('results'),
			'25' 	=> $this->EE->lang->line('show').NBS.'25'.NBS.$this->EE->lang->line('results'),
			'50' 	=> $this->EE->lang->line('show').NBS.'50'.NBS.$this->EE->lang->line('results'),
			'100' 	=> $this->EE->lang->line('show').NBS.'100'.NBS.$this->EE->lang->line('results'),
			'150' 	=> $this->EE->lang->line('show').NBS.'150'.NBS.$this->EE->lang->line('results'),
			'200' 	=> $this->EE->lang->line('show').NBS.'200'.NBS.$this->EE->lang->line('results'),
		);

		//	----------------------------------------
		// 	Entry Date dropdown options
		//	----------------------------------------
		$this->entry_date_dropdown_options = array(
		...
		//	----------------------------------------
		// 	Sticky dropdown options
		//	----------------------------------------
		$this->sticky_options 	= array(
	...
	*/
	function _verify_site_channel($channel_id)
	{
		...
		{
			if( ! array_key_exists($ch_id, $this->assigned_data['channels']) && $ch_id != "0")
			{
	...
	* @return	array
	*/
	function _get_first_and_orderby_dropdown()
	{
		...
		$output['rules_first_dropdown']['ch_id_0']['dropdown_labels'] = $this->first_dropdown;


		//	----------------------------------------
		// 	Dropdown for "order by"
		//	----------------------------------------
		$orderby_dd_array_0 = $this->orderby_options;

		// Add a few dropdown items based on settings
		if(isset($this->settings['setting'][0]['show_autosave']['show']) && $this->settings['setting'][0]['show_autosave']['show'] == 'y')
		{
		...
		if($results->num_rows() > 0)
		{
			...
			foreach($results->result_array() as $row)
			{
				...
				$orderby_dd_array_ch_{$ch_id}	= $this->orderby_options;
				$first_dropdown_{$ch_id}		= $this->first_dropdown;

				//	----------------------------------------
				// 	Add a few dropdown items based on settings
				//	----------------------------------------
				if(isset($this->settings['setting'][$ch_id]['show_autosave']['show']) && $this->settings['setting'][$ch_id]['show_autosave']['show'] == 'y')
				{
				...
				//	----------------------------------------
				// 	Build default dropdowns for each channel
				//	----------------------------------------
				foreach($first_dropdown_{$ch_id} as $key => $val)
				{
					$output['rules_first_dropdown']['ch_id_'.$ch_id]['dropdown_labels'][$key] = $val;
				}

				//	----------------------------------------
				//	Find custom fields to show in dropdown
				//	----------------------------------------
				if(isset($this->settings['setting'][$ch_id]['show_custom_fields']))
				{
					$shown_custom_fields = explode("|", $this->settings['setting'][$ch_id]['show_custom_fields']);

					// Override to show all fields if general setting says so.
					if ( isset($this->settings['setting']['general']['enable_hidden_field_search']) && $this->settings['setting']['general']['enable_hidden_field_search'] == 'y' )
					{
				...
				//	----------------------------------------
				//	Add these custom fields to show to
			...
			foreach($this->assigned_data['channels'] as $ch_id => $ch_name)
			{
				foreach($orderby_dd_array_0 as $key => $lang_key)
				{
					$output['orderby_dropdown']['ch_id_'.$ch_id]['dropdown_labels'][$key] = $this->EE->lang->line($lang_key);
		...
		if ($this->EE->extensions->active_hook('zenbu_add_column') === TRUE)
		{
		 	...
		 	if(is_array($hook_fields_and_labels))
			{
				foreach($hook_fields_and_labels as $key => $fal)
				{
					...
					{
						if(	isset($fal['column']) &&
							(isset($this->settings['setting'][$ch_id][$fal['column']]) && $this->settings['setting'][$ch_id][$fal['column']] == 'y') )
						{
					...
					if(	isset($fal['column']) &&
						(isset($this->settings['setting'][0][$fal['column']]) && $this->settings['setting'][0][$fal['column']] == 'y') )
					{
						$column = (strncmp($fal['column'], 'show_', 5) == 0) ? substr($fal['column'], 5) : $fal['column'];
	...
	*/
	function _get_second_dropdown()
	{
		$output['channel_second_dropdown']['labels'] = $this->channel_second_dropdown;
		$output['second_dropdown_type1']['labels'] = $this->second_dropdown_type1;
		$output['second_dropdown_type2']['labels'] = $this->second_dropdown_type2;
		$output['second_dropdown_type3']['labels'] = $this->second_dropdown_type3;
	...
	function _get_general_form_variables()
	{
		...
		//	Sticky
		//	----------------------------------------
		$vars['sticky']['dropdown_labels'] = $this->sticky_options;

		//	----------------------------------------
		//	Entry date range
		//	----------------------------------------
		$vars['entry_date']['dropdown_labels'] = $this->entry_date_dropdown_options;

		//	----------------------------------------
		//	Limit
		//	----------------------------------------
		$vars['limit']['dropdown_labels'] = $this->limit_dropdown_options;
		...
		{
			$vars['limit']['dropdown_labels'][$max_results_per_page] = $this->EE->lang->line('show').NBS.$max_results_per_page.NBS.$this->EE->lang->line('results');
		}
		...
		//	Sort
		//	----------------------------------------
		$vars['sort']['dropdown_labels'] = $this->sort_options;
	...
	*/
	...
	function get_assigned_data()
	{
		...
		//	Assigned channels
		//	----------------------------------------
		if($this->member_group_id == 1 || $this->cp_call === FALSE)
		{
			// If you're an SA, or you're on the front-end,
			// make all channels accessible: Guests might need
			// this data for front-end manipulations (eg. Hokoku output)
	...
	*/
	...
	function _get_author_dropdowns()
	{
		...
		$results = $this->EE->db->query("/* Zenbu _get_author_dropdowns */ \n
			SELECT c.author_id as member_id, c.channel_id, m.screen_name FROM exp_channel_titles c
			LEFT JOIN exp_members m ON m.member_id = c.author_id
			WHERE c.author_id != 0
			AND c.author_id != ''
			AND c.site_id = " . $this->site_id . "
			GROUP BY c.author_id, c.channel_id
	...
	function _get_status_dropdowns()
	{
		...
		foreach($this->assigned_data['channels'] as $id => $title)
		{
			$channel_id[] = $id;
		}
	...
	function _get_channel_data($member_group_id)
	{
		...
		foreach($this->assigned_data['channels'] as $channel_id => $channel_data)
		{
	...
	*/
	...
	{
		// Return data if already cached
		if($this->EE->session->cache('zenbu', 'field_ids'))
		{
			return $this->EE->session->cache('zenbu', 'field_ids');
		}
		...
		$this->EE->session->set_cache('zenbu', 'field_ids', $output);
	...
	*/
	function _get_basic_template_data($channel_id = "")
	{
		...
		if( ! empty($channel_id) && is_array($channel_id))
		{
			...
			$channel_filter = ($channel_id != "0") ? " AND c.channel_id IN (".$channel_id.")" : '';
		} else {
			$channel_filter = "";
		}

		$query = $this->EE->db->query("/* Zenbu _get_basic_template_data */ \n SELECT c.channel_id, t.template_name, tg.group_name, tg.site_id
			FROM exp_template_groups tg
			JOIN exp_templates t ON t.group_id = tg.group_id
			JOIN exp_channels c ON c.live_look_template = t.template_id
			WHERE c.site_id = " . $this->site_id . $channel_filter);

		if($query->num_rows() > 0)
		{
				foreach($query->result_array() as $row)
				{
					$output[$row['channel_id']]['site_id']			= $row['site_id'];
					$output[$row['channel_id']]['group_name'] 		= $row['group_name'];
					$output[$row['channel_id']]['template_name'] 	= $row['template_name'];
				}
		}
	...
	*/
	...
	function _get_entry_data($settings, $field_ids, $rules, $channel_id, $output_channel, $categories, $fieldtypes)
	{
		...
		$pages_module		= (in_array('Pages', $this->installed_addons['modules'])) ? TRUE : FALSE;
		...
		*/
		...
		if( ! empty($channel_id) && count($channel_id) == 1 )
		{
			...
			$this->EE->db->where_in("channel_titles.channel_id", $this->assigned_data['channel_ids']);
		}
					...
					case 'cat_id':
						...
						*/
						if( is_numeric($cat_id) )
						{
							...
							//
							if(in_array('Category_permissions_ext', $this->installed_addons['extensions']) && $this->member_group_id != 1)
							{
								...
								*/
								...
								if(class_exists($ft_class))
								{
									...
									if(method_exists($ft_object, 'zenbu_result_query'))
									{
										...
										$ft_object->zenbu_result_query($rules, $field_id, $fieldtypes, $$already_queried, $this->installed_addons, $settings['setting'][$channel_id]);
		...
		if($final_results->num_rows() > 0)
		{
				...
				foreach($final_results->result_array() as $row)
				{
					...
					*/
					...
					// Categories
					$output['entry'][$row['entry_id']]['categories'] = $this->EE->zenbu_display->_display_category_list($row['entry_id'], $category_list, $this->installed_addons, $categories, $extra_options);
					/**
					*	=============================================
					...
					*/
					...
					if( ! empty($field_ids))
					{
						foreach($field_ids['field'] as $field_id => $field_label)
						{
							...
							{
								$ft_object = create_object($ft_class);
								$field_data	= (method_exists($ft_object, 'zenbu_display')) ? $ft_object->zenbu_display($row['entry_id'], $channel_id, $row['field_id_'.$field_id], $table_data, $field_id, $settings, $rules, $output_upload_prefs, $this->installed_addons, $fieldtypes) : $row['field_id_'.$field_id];
							} else {
	...
	function _get_single_cell_data($entry_data, $fields)
	{
		...
		if( ! is_numeric($field_id))
		{
			switch($field_id)
			{
				...
				break;
				case 'category':
					// $categories	= $this->EE->zenbu_get->_get_category_dropdowns($this->installed_addons);
					// $category_list 	= $this->_get_category_list(array($entry_data['id']));
					// $output = $this->EE->zenbu_display->_display_category_list($entry_data['id'], $category_list, $this->installed_addons, $categories, $extra_options);
					// /**
					// *	=============================================
					// *	Extension Hook zenbu_modify_category_display
					// *	=============================================
		...
		{
			$ft_object = create_object($ft_class);
			$field_data	= (method_exists($ft_object, 'zenbu_display')) ? $ft_object->zenbu_display($entry_data['id'], $channel_id, $data, $table_data, $field_id, $settings, array(), $output_upload_prefs, $this->installed_addons, $fields) : $data;
		} else {
