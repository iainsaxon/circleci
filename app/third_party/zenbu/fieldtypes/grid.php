...
class Zenbu_grid_ft extends Grid_ft
{
	...
	*/
	function zenbu_display($entry_id, $channel_id, $data, $grid_data = array(), $field_id, $settings, $rules = array(), $upload_prefs = array(), $installed_addons)
	{
		...
		/**
		* Displaying the Grid inline or as a fancybox link
		...
		*/
		...
		if( ! isset($extra_options["field_".$field_id]['grid_option_1']))
		{
			...
			$link_to_grid = '#e_'.$entry_id.'_f_'.$field_id;
			return anchor($link_to_grid, $this->EE->lang->line('show_').$this->EE->lang->line('grid'), 'class="fancyboxmatrix"') . $output;
		} else {
	...
	*/
	function zenbu_result_query($rules = array(), $field_id = "", $fieldtypes, $already_queried = FALSE)
	{
		...
		$col_query = $this->EE->db->query("/* Zenbu: Show columns for Grid */\nSHOW COLUMNS FROM exp_channel_grid_field_".$field_id);
		$concat = "";
		$where_in = array();
		...
		if( ! empty($concat))
		{
			...
			foreach($rules as $rule)
			{
				...
				if(isset($fieldtypes['fieldtype'][$rule_field_id]) && $fieldtypes['fieldtype'][$rule_field_id] == "grid")
				{
					$keyword = $rule['val'];

					$keyword_query = $this->EE->db->query("/* Zenbu: Search Grid */\nSELECT entry_id FROM exp_channel_grid_field_".$field_id." WHERE \nCONCAT_WS(',', ".$concat.") \nLIKE '%".$this->EE->db->escape_like_str($keyword)."%' GROUP BY entry_id");
