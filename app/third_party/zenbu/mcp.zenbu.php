...
class Zenbu_mcp {
	...
	function index($limit = '', $perpage = '')
	{
		...
		*/
		if( empty($this->assigned_data['channels']) && $this->cp_call === TRUE )
		{
		...
		$max_results_per_page = (isset($this->settings['setting']['general']['max_results_per_page'])) ? $this->settings['setting']['general']['max_results_per_page'] : '';
		$default_filter       = (isset($this->settings['setting']['general']['default_1st_filter'])) ? $this->settings['setting']['general']['default_1st_filter'] : "cat_id";
		$default_order        = (isset($this->settings['setting']['general']['default_order'])) ? $this->settings['setting']['general']['default_order'] : "entry_date";
		$default_sort         = (isset($this->settings['setting']['general']['default_sort'])) ? $this->settings['setting']['general']['default_sort'] : "desc";
		...
		*/
		if($this->EE->input->get_post('rule_id', TRUE))
		{
			...
			foreach($other_rules as $elem => $var)
			{
				if(isset($rules[$elem]))
				{
					...
					$this->settings['setting']['general'][$var] = $rules[$elem];
			...
			foreach($other_rules as $elem => $var)
			{
				...
				{
					// This will be passed in entry_get function
					$this->settings['setting']['general'][$var] = $rule_data[$elem];
					// This is used to refresh the default values above
					$$var = $rule_data[$elem];
				}
			}
			...
			if(empty($rule_data))
			{
				...
				foreach($other_rules as $elem => $var)
				{
					...
					{
						// This will be passed in entry_get function
						$this->settings['setting']['general'][$var] = unserialize($_SESSION['zenbu'][$elem]);
						// This is used to refresh the default values above
		...
		foreach($channel_id as $key => $channel_id_val)
		{
			...
			if( ! isset($this->settings['setting'][$channel_id_val]) )
			{
				$this->settings['setting'][$channel_id_val] = array(
						'show_id' => 'y',
						'show_title' => 'y',
						'field_order' => array(
							'show_id' => 1,
			...
			if($channel_id_val != "" && isset($this->settings['setting'][$channel_id_val]['field_order']))
			{
				$field_order = $this->settings['setting'][$channel_id_val]['field_order'];
			} else {
				$field_order = isset($this->settings['setting']['0']['field_order']) ? $this->settings['setting']['0']['field_order'] : array();
			}
			$field_order = array_flip($field_order);
			$vars_order['field_order'] = $field_order;

			// Get field_ids of fields to show
			$fields_to_show = (isset($this->settings['setting'][$channel_id_val]['show_custom_fields']) && !empty($this->settings['setting'][$channel_id_val]['show_custom_fields'])) ? explode('|', $this->settings['setting'][$channel_id_val]['show_custom_fields']) : array(); // array of field set to be shown
		...
		$vars_categories	= $this->EE->zenbu_get->_get_category_dropdowns($this->installed_addons);
		...
		if(AJAX_REQUEST)
		{
			...
			$vars_first_dropdown_labels				= array($this->settings);
			...
			$vars_first_dropdown_labels				= $this->EE->zenbu_get->_get_first_and_orderby_dropdown();
		...
		* ====================================
		*/
		$vars_entries = $this->EE->zenbu_get->_get_entry_data($this->settings, $vars_fields, $rules, $channel_id, $vars_channels, $vars_categories, $fields_from_channel); // THE function
		...
		*/
		...
		} elseif(isset($this->settings['setting']['general']['default_limit'])) {

			$vars_rules['limit_val'] = $this->settings['setting']['general']['default_limit'];
		...
		*/
		...
		if($this->cp_call)
		{
			...
			if($this->settings['setting']['can_access_settings'] == 'y' || $this->member_group_id == 1) {
			...
			if($this->settings['setting']['can_admin'] == 'y' || $this->member_group_id == 1) {
	...
	*/
	function multi_edit()
	{
		...
		case "add_categories" OR "remove_categories":
			...
			$vars['category_group_names'] = isset($category_data['category_groups']) ? $category_data['category_groups'] : array();
	...
	function manage_searches()
	{
		...
		*/
		...
		//	----------------------------------------
		if($this->settings['setting']['can_view_group_searches'] == 'y' && $this->settings['setting']['can_admin_group_searches'] == 'n')
		{
	...
	function settings()
	{
		...
		//	----------------------------------------
		//	After saving settings, remove settings
		//	from cache, and get fresh data
		//	----------------------------------------
		$this->EE->session->set_cache('zenbu', 'settings', '');
		$this->settings = $this->EE->zenbu_get->_get_settings();
		...
		$comment_module = (in_array('Comment', $this->installed_addons['modules'])) ? TRUE : FALSE;
		...
		*/
		if ($this->EE->extensions->active_hook('zenbu_add_column') === TRUE)
		{
		 	...
		 	if(is_array($hook_fields_and_labels))
			{
				foreach($hook_fields_and_labels as $key => $fal)
				{
					...
					$third_party_columns[$fal['column']] = $fal['column'];
				}
			}
		}
		...
		foreach($vars_channels['channels']['channel_data'] as $channel_id => $value)
		{
			...
			$livelook_template_array = $this->EE->zenbu_get->_get_basic_template_data($channel_id_array);
			...
			// Set field order
			$field_order = (isset($this->settings['setting'][$channel_id]['field_order'])) ? array_merge($field_order_std, $this->settings['setting'][$channel_id]['field_order']) : $field_order_std;

			//	----------------------------------------
			//	If field_order comes from stored settings,
			//	and 3rd-party column isn't present,
			//	add the column to $field_order
			//	----------------------------------------

			if( isset($third_party_columns) )
			{
				foreach($third_party_columns as $third_party_column)
				{
					if( ! isset($field_order[$third_party_column]) )
					{
						$field_order[$third_party_column] = count($field_order) + 1;
					}
				}
			}
			...
			// Set extra options
			$extra_options = (isset($this->settings['setting'][$channel_id]['extra_options'])) ? $this->settings['setting'][$channel_id]['extra_options'] : array();

			// Get array of custom fields to show
			if(isset($this->settings['setting'][$channel_id]['show_custom_fields']) && ! empty($this->settings['setting'][$channel_id]['show_custom_fields']))
			{
				$fields_to_show = explode('|', $this->settings['setting'][$channel_id]['show_custom_fields']); // array of field set to be shown
			} else {
			...
			foreach($field_order as $table_col => $order)
			{
				if(substr($table_col, 0, 6) == 'field_')
				{
					...
					if(in_array(substr($table_col, 6), $field_id_array)) // Checks if field still exists and compares with stored settings
					{
						...
						if(in_array('Pages', $this->installed_addons['modules']))
						{
							$this->non_ft_extra_options['livelook_option_4'] = 'livelook_option_4';
						}

						// Build the completed "extra_settings" array, to be looped in settings view
						if(isset($vars_labels['extra_settings']))
						{
							$vars_labels['extra_settings'] = array_merge($vars_labels['extra_settings'], $this->non_ft_extra_options);
						}
					}
				...
				else
				{
					...
					{
						$order_num = $field_order[$table_col];
					}
					else if( ! array_key_exists($table_col, $field_order))
					{
					...
					// Standard fields
					if( isset($labels_std[$table_col]) || isset($field_order_std[$table_col]))
					{
						...
						$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]['option_title'] = isset($labels_std[$table_col]) ? $labels_std[$table_col] : $field_order_std[$table_col];
						$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]['checked'] = (isset($this->settings['setting'][$channel_id][$table_col]) && $this->settings['setting'][$channel_id][$table_col] == 'y') ? TRUE : FALSE;
					}
					...
					switch ($table_col)
					{
						case "show_categories";
							...
							$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]['category_option_1'] = form_label($this->EE->lang->line('no_categories_to_display').NBS.form_input('settings['.$channel_id.']['.$table_col.'][category_option_1]', $extra_category_option_1, 'size="2" maxlength="3"'));
						break;
						case "show_view_count":
							...
							foreach($option_view_counter_array as $label => $lang_string)
							{
								...
								$vars_labels['setting_labels'][$channel_id][$order_num][$table_col][$label] = form_label(form_checkbox('settings['.$channel_id.']['.$table_col.']['.$label.']', 'y', $$checked_show_view_count).NBS.$this->EE->lang->line('show_view_count').' '.substr($label, 11)).'<br />';
							}
						break;
						case ($table_col == "show_entry_date" || $table_col == "show_expiration_date" || $table_col == "show_edit_date"):
							...
							$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]["date_option_1"] = form_label($this->EE->lang->line('date_format').NBS.form_input('settings['.$channel_id.']['.$table_col.'][date_option_1]', $extra_date_option_1, 'size="20" class="bottom-margin"'));
							if($table_col != "show_edit_date")
							{
								...
								$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]["date_option_2"] = BR . form_label($this->EE->lang->line('date_format_future').NBS.form_input('settings['.$channel_id.']['.$table_col.'][date_option_2]', $extra_date_option_2, 'size="20"'));
							}
						break;
						case "show_view":
							...
							if($channel_id != "0")
							{
								$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]["livelook_option_1"] = form_dropdown('settings['.$channel_id.']['.$table_col.'][livelook_option_1]', $livelook_options_dropdown, $extra_livelook_option_1, 'class="livelook-settings bottom-margin"' ) . $livelook_template;

								// Reset for next loop
								$livelook_options_dropdown = array();

								// Build custom segments when single channels are displayed
								$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]["livelook_option_2"] = form_label($this->EE->lang->line('custom_segments') . NBS . form_input('settings['.$channel_id.']['.$table_col.'][livelook_option_2]', $extra_livelook_option_2, 'size="20" id="" ' . $disabled ) . ' /', '', $disabled_arr);

								$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]["livelook_option_3"] = form_dropdown('settings['.$channel_id.']['.$table_col.'][livelook_option_3]', $livelook_options_dropdown2, $extra_livelook_option_3, 'class="bottom-margin"' );

								// Add option for Pages module override if Pages module is installed
								if(in_array('Pages', $this->installed_addons['modules']))
								{
									$vars_labels['setting_labels'][$channel_id][$order_num][$table_col]["livelook_option_4"] = '<br />'.form_label(form_checkbox('settings['.$channel_id.']['.$table_col.'][livelook_option_4]', 'y', $extra_livelook_option_4) . NBS . $this->EE->lang->line('livelook_pages_override'));
								}
						...
						default:
							...
							*/
							...
							//	----------------------------------------
							//  The following code, which allows adding settings in
							//  "extra settings" column in Zenbu's Display Settings,
							//  is not as elegant as hoped for, because add-ons using
							//  this part of the hook need to create form fields with,
							//  to me, too complex name="" attributes.
							//  Disabled for now.
							//	----------------------------------------
							...
							// 		foreach($hook_fields_and_labels as $key => $fal)
							// 		{
							// 			if(isset($fal['settings']) && $fal['column'] == $table_col)
							// 			{
							// 				foreach($fal['settings'] as $setting_key => $setting_val)
							// 				{
							// 					$vars_labels['setting_labels'][$channel_id][$order_num][$table_col][$setting_key] = $setting_val;
							// 					$vars_labels['extra_settings'] = array_merge($vars_labels['extra_settings'], array($setting_key => $setting_key));
							// 				}
							// 			}
							// 		}
							// 	}
		...
		*/
		...
		$default_1st_filter = (isset($this->settings['setting']['general']['default_1st_filter'])) ? $this->settings['setting']['general']['default_1st_filter'] : '';
		...
		$default_limit = (isset($this->settings['setting']['general']['default_limit'])) ? $this->settings['setting']['general']['default_limit'] : $this->default_limit;
		...
		$vars_labels['general_settings'][3]['form_field'] = form_dropdown('settings[general][default_order]', $default_ordering_array, $default_order)
			.NBS.form_dropdown('settings[general][default_sort]', $default_sorting_array, $default_sort);

		//	----------------------------------------
		//	Show in dropdown and be able to search all custom fields
		//	----------------------------------------
		$enable_hidden_field_search_y = (isset($this->settings['setting']['general']['enable_hidden_field_search']) && $this->settings['setting']['general']['enable_hidden_field_search'] == 'y') ? TRUE : FALSE;

		$enable_hidden_field_search_n = (isset($this->settings['setting']['general']['enable_hidden_field_search']) && $this->settings['setting']['general']['enable_hidden_field_search'] == 'n') ? TRUE : FALSE;
		...
		if($this->settings['setting']['can_copy_profile'] == 'y') {
		...
		$vars_other['can_admin'] = $this->settings['setting']['can_admin'];

		$vars_other['current_member_group'] = $this->member_group_id;

		$vars = array_merge($vars_channels, $vars_labels, $this->settings, $vars_urls, $vars_members, $vars_other);
	...
	function settings_admin()
	{
		...
		if($results->num_rows() > 0) {
			foreach($results->result_array() as $num => $row)
			{
				...
				$this->settings_query = $this->EE->db->query("/* Zenbu get settings for a specific member group/site */ SELECT * FROM exp_".$this->addon_short_name." WHERE member_group_id = ".$row['group_id']." AND site_id = ".$this->site_id);

				if($this->settings_query->num_rows() > 0) {

					foreach($this->settings_query->result_array() as $num2 => $row2)
					{
		...
		{
			$module_id			= array_search(ZENBU_NAME, $this->installed_addons['modules']);
